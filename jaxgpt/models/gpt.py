...
class GPT(nn.Module):
    ...
    def __call__(self, idx: jax.Array, train: bool) -> jax.Array:

        B, T = idx.shape
        assert (
            T <= self.config.block_size
        ), f"Cannot forward sequence of lenght {T} when block size is only {self.config.block_size}"
    ...
    def from_pretrained(cls, model_type, override_args: Optional[dict] = None):
        ...
        def copy_from(
            flax_name, pt_name, transpose=False, add_head_dim=False, debug=True
        ):
            if debug:
                print(
                    f"copying {colored(pt_name, 'red')} -> {colored(flax_name, 'green')} :: {colored_bool(transpose, 'T')} :: {colored_bool(add_head_dim, 'H')}"
                )
    ...
    def create_train_state(
        ...
        **kwargs,
        ...
        if params is None:
            variables = self.init(
                jax.random.PRNGKey(SEED), jnp.ones((1, 1), dtype=jnp.int32), train=False
            )
            params = variables["params"]
        ...
        tx = optax.chain(
            ...
            self.configure_optimizers(
                params, weight_decay, lr_schedule, betas=(beta1, beta2)
            ),
        )
